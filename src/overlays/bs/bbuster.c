...
void bsbbuster_update(PlayerState *self) {
    ...
    switch (self->unk160.bytes[1]) {
        case 1:
            ...
            if (((baanim_isAt(self, 0.3637f) && bakey_held(self, BUTTON_Z)) || !ability_getValue(ABILITY_02_BK_BEAK_BUSTER)) && ability_getValue(ABILITY_19_BILL_DRILL))
            {
